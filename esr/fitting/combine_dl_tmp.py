...
def main(comp, likelihood, print_frequency=1000):
    ...
    with codelen_file.open("r") as f, open(aifeyn_file, "r") as aifeyn_f, open(
        allfn_file, "r"
    ) as allfn_f:
    ...
    with open(output_file, "w") as fout, open(output_file_fcn, "w") as fout_fcn:
        for idx in range(data_start, data_end):
            if idx in results:
                ...
                line_data = (
                    [row[0]] + params + [0.0] * (num_params - len(params)) + row[-3:]
                )
                fcn = results_fcn[idx]
            else:
    ...
    if rank == 0:
        ...
        data_entries = []
        num_params = 0
        with open(fit_paths["combined"], "r") as f, open(
            fit_paths["combined_functions"], "r"
        ) as fcn_f:
        ...
        print(
            f"Sorted {len(data_entries)} entries by DL for complexity {comp}",
            flush=True,
        )
