...
def main(
    ...
    expand_tmax=1,
    seed=1234,
    diagnose_numerical_duplicates=False,
    fn_dir=None,
):
    ...
    if fn_dir is None:
        dirname = os.path.abspath(
            os.path.join(os.path.dirname(generator.__file__), "..", "function_library")
        )
    ...
    if nextra > 0:
        ...
        all_fun[:-nextra], all_sym = simplifier.initial_sympify(
            all_fun[:-nextra], max_param, track_memory=track_memory
        )

        # Get str but not sympy of the extra equations
        all_fun[-nextra:], _ = simplifier.initial_sympify(
            all_fun[-nextra:],
            max_param,
        ...
        all_fun, all_sym = simplifier.initial_sympify(
            all_fun, max_param, track_memory=track_memory
        )
    ...
    all_fun, _, nround = simplifier.do_sympy(
        all_fun,
        all_sym,
        compl,
        search_tmax,
    ...
    if rank == 0:
        ...
        for i in range(max_param + 1):
            if i == 1:
                print(
                    f"Functions with 1 parameter: {np.sum(uniq_nparam == 1)} ({nparam[1]})"
                )
            else:
                print(
                    f"Functions with {i} parameters: {np.sum(uniq_nparam == i)} ({nparam[i]})"
                )
    ...
    for r in range(nround):
        ...
        inv = simplifier.load_subs(
            f"{dirname}/inv_subs_{compl}_round_{r}.txt", max_param, use_sympy=False
        )

        if rank == 0:
            if len(inv) != 0:
                idx = np.atleast_1d(
                    np.loadtxt(f"{dirname}/inv_idx_{compl}_round_{r}.txt", dtype=int)
                )
            else:
                idx = []
    ...
    if rank == 0 and diagnose_numerical_duplicates:
        ...
        with utils.atomic_write(report_file) as f:
            ...
            f.write(
                "# Verify exact equality over the required variable and "
