...
def test_cc(monkeypatch):
    ...
    # test using labels
    nll_0, DL_0 = single_function(labels, basis_functions, likelihood, verbose=True)
    aifeyn_0, comp_0 = tree_to_aifeyn(labels, basis_functions)

    # test using string
    fun = "a0 + a1 * x**3"
    nll_1, DL_1, labels_1 = fit_from_string(
        fun,
        basis_functions,
        likelihood,
    )
...
def test_pantheon(monkeypatch):
    ...
    assert (
        esr.plotting.plot.pareto_plot(
            likelihood.out_dir, "no_name", do_DL=False, do_logL=False
        )
        is None
    )
    esr.plotting.plot.pareto_plot(
...
def test_mse():
    ...
    unittest.TestCase().assertRaises(
        ValueError, esr.fitting.test_all_fisher.main, comp=comp, likelihood=likelihood
    )
...
def test_snap_choices():
    ...
    for sc in [0, 1, 2]:
        for det_I in [True, False]:
            ...
            assert np.isfinite(
                nll
            ), f"snap_choice={sc}, use_det_I={det_I}: nll not finite"
            assert np.isfinite(
                DL
            ), f"snap_choice={sc}, use_det_I={det_I}: DL not finite"
    ...
    assert np.allclose(
        nlls, nlls[0], atol=1e-4
    ), f"negloglike differs across snap_choices: {nlls}"
...
def test_compute_codelen():
    ...
    theta = np.array([5.0])
    mask = np.array([True])
    expected = -0.5 * math.log(3.0) + 0.5 * math.log(100.0) + math.log(5.0)
    ...
    assert np.isclose(
        cl_det, cl_diag
    ), f"Diagonal Hessian: det={cl_det}, diag={cl_diag}"
    ...
    expected_det = (
        -1.0 * math.log(3.0) + 0.5 * math.log(7500.0) + math.log(3.0) + math.log(7.0)
    )
    ...
    H_even_saddle = np.array([[1.0, 2.0, 2.0], [2.0, 1.0, 2.0], [2.0, 2.0, 1.0]])
    assert (
        _compute_codelen(
            H_even_saddle,
            np.diag(H_even_saddle),
            np.ones(3),
    ...
    theta_small = np.array([1.0e-8])
    assert _compute_codelen(
        np.array([[100.0]]), np.array([100.0]), theta_small, np.array([True]), True
    ) > _compute_codelen(
        np.array([[100.0]]), np.array([100.0]), theta_small, np.array([True]), False
    )

    # Partial mask: only keep first parameter
    mask1 = np.array([True, False])
    cl = _compute_codelen(H, diag, theta, mask1, True)
    expected_1 = -0.5 * math.log(3.0) + 0.5 * math.log(100.0) + math.log(3.0)
...
def test_compute_snap_mask():
    ...
    Nsteps_diag = np.abs(theta) / np.sqrt(12.0 / diag)  # both >> 1
    for sc in [0, 1]:
        result, degen = _compute_snap_mask(H, diag, theta, Nsteps_diag.copy(), sc)
        assert np.all(
            result >= 1
        ), f"snap_choice={sc}: should not snap well-constrained params"
        if sc == 1:
            assert (
                not degen
            ), f"snap_choice={sc}: well-conditioned Hessian should not be degenerate"
    ...
    Nsteps_diag_small = np.abs(theta_small) / np.sqrt(12.0 / diag_degen)
    ...
    Nsteps_npd = np.abs(theta_npd) / np.sqrt(12.0 / diag_npd)
    result, degen = _compute_snap_mask(
...
def test_fitting_path_helpers_centralize_shared_filenames(tmp_path):
    ...
    assert paths["negloglike"] == str(tmp_path / "output" / "negloglike_comp6.dat")
    ...
    assert paths["codelen"] == str(tmp_path / "output" / "codelen_comp6_deriv.dat")
    assert paths["derivs"] == str(tmp_path / "output" / "derivs_comp6.dat")
    assert paths["codelen_matches"] == str(
        tmp_path / "output" / "codelen_matches_comp6.dat"
    )
    assert paths["combined"] == str(tmp_path / "output" / "combined_comp6.dat")
    assert paths["combined_functions"] == str(
        tmp_path / "output" / "combined_fcn_comp6.dat"
    )
    assert paths["final"] == str(tmp_path / "output" / "ranked_6.dat")
    assert paths["results_pretty"] == str(tmp_path / "output" / "results_pretty_6.txt")
    assert paths["negloglike_rank"] == str(
        tmp_path / "partial" / "chi2_comp6weights_3.dat"
    )
    assert paths["codelen_rank"] == str(tmp_path / "partial" / "codelen_deriv_6_3.dat")
    assert paths["derivs_rank"] == str(tmp_path / "partial" / "derivs_6_3.dat")
    assert paths["codelen_matches_rank"] == str(
        tmp_path / "partial" / "codelen_matches_6_3.dat"
    )
    assert paths["combined_rank"] == str(tmp_path / "partial" / "combined_6_3.dat")
...
def test_likelihood_aware_catalogue_groups_transformed_models(tmp_path):
    ...
    assert test_all.ensure_likelihood_catalogue(
...
def test_likelihood_aware_match_uses_transformed_representatives(tmp_path):
    ...
    np.savetxt(
        tmp_path / "out" / f"negloglike_comp{comp}.dat",
        np.array([[10.0, 2.0, 0.0, 0.0, 0.0], [20.0, 3.0, 0.0, 0.0, 0.0]]),
    )
    np.savetxt(
        tmp_path / "out" / f"codelen_comp{comp}_deriv.dat",
        np.array([[1.5, 10.0, 2.0, 0.0, 0.0, 0.0], [2.5, 20.0, 3.0, 0.0, 0.0, 0.0]]),
    )
...
def test_unresolved_curvature_is_scored_independently_of_its_sign():
    ...
    assert _has_negative_curvature(almost_singular(100.0 * EIGENVALUE_REL_THRESHOLD))
    ...
    for excess in (+unresolved, -unresolved):
        ...
        diag = np.diag(H)
        Nsteps = np.abs(theta) / np.sqrt(12.0 / diag)
    ...
    scores = [
        _score_projected_eigenbasis(
            almost_singular(excess), theta, 100.0, True, flat_negloglike
        )[3]
        for excess in (+unresolved, -unresolved)
    ]
...
def test_likelihood_catalogue_keeps_the_simplifiers_representatives(tmp_path):
    ...
    assert (
        "pow(x,(a0*a1))" not in representatives
...
def _straight_line_data(tmp_path, name="invariance.txt"):
    ...
    likelihood = GaussLikelihood(
        name, "invariance", data_dir=str(tmp_path), base_out_dir=str(tmp_path)
    )
...
def _parametric_codelen(fcn, theta, likelihood, use_det_I, snap_choice=1):
    ...
    syms = list(
        np.atleast_1d(
            sympy.symbols(" ".join(f"a{i}" for i in range(len(theta))), real=True)
        )
    )
...
def test_determinant_codelen_is_the_same_for_equivalent_parameterisations(tmp_path):
    ...
    determinant = {
        f: _parametric_codelen(f, t, likelihood, True) for f, t in forms.items()
    }
    diagonal = {
        f: _parametric_codelen(f, t, likelihood, False, snap_choice=0)
    ...
    assert diag_spread > 0.1, f"diagonal codelengths unexpectedly agree: {diagonal}"
...
def test_diagonal_codelen_exceeds_determinant_by_the_correlation_term(tmp_path):
    ...
    measured = _parametric_codelen(
        "a0 + a1*x", theta, likelihood, False, 0
    ) - _parametric_codelen("a0 + a1*x", theta, likelihood, True)

    assert predicted > 0  # Hadamard
    assert np.isclose(
        measured, predicted, atol=1e-4
...
def test_unremovable_degenerate_direction_gives_an_infinite_codelen(tmp_path):
    ...
    np.savetxt(str(tmp_path / "degenerate.txt"), np.array([xvar, yvar, yerr]).T)
...
def _null_slope_fit(tmp_path, offset):
    ...
    np.savetxt(
        str(tmp_path / "null_slope.txt"), np.array([xvar, a0_ml * xvar + noise, yerr]).T
    )
    likelihood = GaussLikelihood(
        "null_slope.txt",
        "null_slope",
...
def test_published_diagonal_snaps_every_parameter_below_one_precision_step(
    ...
    if offset < np.sqrt(12.0):
        ...
        assert np.isclose(codelen, np.log(offset) - 0.5 * np.log(3.0), atol=1e-4)
        assert np.isclose(params[0], a0_ml)
        assert np.isclose(negloglike, negloglike_ml)
...
def test_likelihood_catalogue_cache_invalidates_on_grouping_change(tmp_path):
    ...
    assert (
        second["settings"]["raw_matches_hash"] != first["settings"]["raw_matches_hash"]
    )
    with open(paths["matches"]) as f:
        matches = [int(v) for v in f.read().split()]
    assert len(matches) == len(all_functions)
...
def test_degeneracy_verdict_does_not_depend_on_parameter_scaling():
    ...
    for stretch in [1.0, 1.0e3, 1.0e-3, 1.0e6]:
        ...
        Nsteps = np.abs(theta_scaled) / np.sqrt(12.0 / diag)
        _, degenerate = _compute_snap_mask(
            H_scaled, diag, theta_scaled, Nsteps.copy(), 1
        )
...
def test_numerical_fingerprint():
    ...
    groups = numerical_duplicate_candidates(diagnostic_only, max_param=2, verbose=False)
...
def test_numerical_duplicate_diagnostic_does_not_change_catalogue():
    ...
    report_path = os.path.join(fn_dir, f"numerical_duplicate_candidates_{comp}.txt")
...
def _combine_dl_test_likelihood(tmp_path, codelen_lines, aifeyn_lines, function_lines):
...
def test_combine_dl_warns_on_malformed_rows_and_preserves_parameter_width(
    ...
    monkeypatch.setattr(
        esr.fitting.combine_dl.test_all,
        "get_functions",
...
def test_combine_dl_clears_stale_final_when_no_rows_are_valid(tmp_path, monkeypatch):
    comp, likelihood = _combine_dl_test_likelihood(tmp_path, "nan 2 0\n", "1\n", "f0\n")
...
def test_convert_params_reconstructs_known_hessian_from_data(tmp_path):
    ...
    likelihood = GaussLikelihood("data.txt", "known_hessian", data_dir=str(tmp_path))
...
def test_determinant_survives_parameter_removal(tmp_path):
    ...
    cases = [
        (
            ...
            [np.ones_like(xvar), xvar, xvar**2],
            [1.0, 2.0, 0.5],
        ),
        (
            sympy.symbols("a0 a1 a3 a5", real=True),
            [np.ones_like(xvar), xvar, xvar**2, xvar**3],
            [1.0, 2.0, 0.5, -0.3],
        ),
    ]
    ...
    for syms, basis, coeffs in cases:
        ...
        np.savetxt(tmp_path / "data.txt", np.column_stack([xvar, yvar, yerr]))
        likelihood = GaussLikelihood(
            "data.txt", f"removal_{nparam}", data_dir=str(tmp_path)
        )
...
def test_plot_uses_the_parameterisation_the_fit_was_stored_in(tmp_path, monkeypatch):
    ...
    likelihood = OffsetRemovingGauss(
        "data.txt", "plot_basis", data_dir=str(tmp_path), base_out_dir=str(tmp_path)
    )
    ...
    esr.fitting.test_all_fisher.main(comp, likelihood, use_det_I=True, snap_choice=1)
    esr.fitting.match.main(comp, likelihood)
    esr.fitting.combine_dl.main(comp, likelihood)
    ...
    np.testing.assert_allclose(
        ypred, measured[0] * (likelihood.xvar - 1.0), rtol=1e-6, atol=1e-8
    )
    assert not np.allclose(ypred, 0.0)  # what the raw parameter count gives


def test_determinant_scoring_and_matching_with_parameter_removal(tmp_path, monkeypatch):
    ...
    likelihood = OffsetRemovingGauss("data.txt", "det_removal", data_dir=str(tmp_path))
    likelihood.fn_dir = str(tmp_path / "functions")
    ...
    matched = np.atleast_2d(
        np.loadtxt(os.path.join(likelihood.out_dir, f"codelen_matches_comp{comp}.dat"))
    )
...
def _fitted_cc_pipeline(tmp_path, comp=3):
    ...
    likelihood = CCLikelihood(
        fn_dir=str(tmp_path / "functions"), base_out_dir=str(tmp_path / "output")
    )
    esr.generation.duplicate_checker.main("core_maths", comp, fn_dir=likelihood.fn_dir)
    esr.fitting.test_all.main(comp, likelihood)
    esr.fitting.test_all_fisher.main(comp, likelihood, use_det_I=True, snap_choice=1)
    return likelihood
...
def test_dynamic_fitting_records_the_catalogue_digest(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        esr.fitting.test_all, "_main_dynamic", lambda *a, **k: called.append(True)
    )
    esr.fitting.test_all.main(comp, likelihood)
...
def test_interrupted_fisher_run_leaves_no_scoring_settings(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(esr.fitting.test_all_fisher, "combine_temp_files", interrupted)
    with pytest.raises(RuntimeError, match="interrupted"):
        esr.fitting.test_all_fisher.main(
            comp, likelihood, use_det_I=False, snap_choice=0
        )
    assert esr.fitting.test_all_fisher.load_scoring_settings(comp, likelihood) is None
    with pytest.raises(ValueError, match="No saved Fisher settings"):
        esr.fitting.match.main(comp, likelihood)


def test_legacy_diagonal_settings_reproduce_published_values(monkeypatch, tmp_path):
    ...
    esr.fitting.test_all.main(comp, likelihood)
    esr.fitting.test_all_fisher.main(comp, likelihood, use_det_I=False, snap_choice=0)
    esr.fitting.match.main(comp, likelihood)
    ...
    assert np.isclose(
        float(best[2]), float(best[4]) + float(best[5]) + float(best[6]), atol=2e-2
    )
...
def test_projected_eigenbasis_snaps_weak_direction():
    ...
    theta_f, nll_f, k, cl = _score_projected_eigenbasis(H, theta, 12.0, True, eval_nll)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            [[4.0, 1.0, 0.0], [1.0, 2.0, 0.5], [0.0, 0.5, 0.0]],
            True,
        ),
    ],
)
...
def test_snap_choice_2_end_to_end(monkeypatch, tmp_path):
    ...
    esr.fitting.test_all.main(comp, likelihood)
    esr.fitting.test_all_fisher.main(comp, likelihood, use_det_I=True, snap_choice=2)
    esr.fitting.match.main(comp, likelihood)
    assert esr.fitting.match.check_match_results(comp, likelihood) == 0
    esr.fitting.combine_dl.main(comp, likelihood)
...
def test_snap_choice_2_match_multiparam_end_to_end(monkeypatch, tmp_path):
    ...
    assert (
        calls["multi"] > 0
    )  # a multi-parameter function's projected coordinate was re-evaluated
...
def test_projected_eigenbasis_correlated_scoring(tmp_path):
    ...
    Gram = _gaussian_design_hessian(basis, yerr)
    w = 1.0 / yerr**2
    theta_mle = np.linalg.solve(Gram, (np.column_stack(basis) * w[:, None]).T @ yvar)
    eq_numpy = sympy.lambdify([xsym, a0s, a1s], eq, "numpy")
    nll = likelihood.negloglike(theta_mle, eq_numpy)

    common = ("a0*x + a1*x**2", eq, False, np.pad(theta_mle, (0, 2)), likelihood, nll)
...
def test_duplicate_checker_and_likelihood_share_custom_fn_dir(tmp_path):
    ...
    esr.generation.duplicate_checker.main("core_maths", 3, fn_dir=likelihood.fn_dir)
...
def test_likelihood_catalogue_cache_invalidates_on_transform_change(tmp_path):
    ...
    class Identity(Base):
        def run_sympify(self, fcn_i, **kwargs):
            ...
            return (
                fcn_i,
                sympy.sympify(fcn_i, locals={"x": x, "a0": a0, "a1": a1, "a2": a2}),
                False,
            )
...
def test_variant_negloglike_reevaluates_correctly(tmp_path):
    ...
    xvar = np.linspace(0.5, 3.0, 40)
    yerr = np.full_like(xvar, 0.5)
    np.savetxt(tmp_path / "data.txt", np.column_stack([xvar, 1.0 + 2.0 * xvar, yerr]))
...
def test_likelihood_catalogue_retries_after_failed_transforms(tmp_path):
    ...
    assert test_all._read_likelihood_catalogue_metadata(comp, lik)["failed_count"] == 1
...
def test_projected_eigenbasis_warns_on_degenerate_hessian():
    ...
    with pytest.warns(ProjectedEigenbasisWarning, match="degenerate"):
        _score_projected_eigenbasis(
            np.diag([100.0, 100.0]), np.array([3.0, 5.0]), 7.0, True, lambda t: 7.0
        )
...
def test_versionless_transform_is_not_cached(tmp_path, monkeypatch):
    ...
    lik = Versionless()
    with pytest.warns(
        test_all.LikelihoodCatalogueWarning, match="catalogue_transform_version"
    ):
        test_all.ensure_likelihood_catalogue(comp, lik, tmax=5)
